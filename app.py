...
Data sources:
  ...
  - FanGraphs (optional, scraped) - Stuff+/Location+/Pitching+

When real data isn't available, cells show "—". Projections (HR Game%,
Proj K) skip players with insufficient sample.
...
from data_fetcher import (
    ...
    get_sprint_speed, get_pitch_run_values,
)
...
from fangraphs import get_fangraphs_pitch_models, merge_fangraphs_into_pitchers


st.set_page_config(page_title="Posey MLB HR & K Data", layout="wide", page_icon="⚾")
st.markdown("<style>div[data-testid='stMetric']{min-height:85px;}</style>",
...
def has_sufficient_pa(row, min_pa=100):
    """Check if a hitter row has enough real PA for projections."""
    pa = row.get("pa") if isinstance(row, dict) else row.get("pa", None)
    if pa is None or pd.isna(pa):
        return False
    try:
        return float(pa) >= min_pa
    except (TypeError, ValueError):
        return False
...
with st.sidebar:
    ...
    use_umpire = st.checkbox("Umpire (Stats API)", value=False,
        help="HP umpire data when available")
    use_recent_form = st.checkbox("Recent form (slower)", value=False)
    use_fangraphs = st.checkbox("FanGraphs Stuff+/Location+/Pitching+", value=False,
        help="Adds FanGraphs pitcher quality models. May fail if site layout changes.")
    use_sprint_speed = st.checkbox("Sprint Speed (Statcast)", value=True)
    use_bvp = st.checkbox("BvP supplemental", value=False)

    st.markdown("### Filter")
    min_pa_filter = st.slider("Min PA for HR projection", 0, 250, 100,
        help="Hitters below this PA show no HR projection (insufficient sample)")
    hide_no_data = st.checkbox("Hide players with no real stats", value=False,
        help="If enabled, hitters with zero PA are dropped from tables")
...
# ===========================================================================
# LOAD DATA - all from real sources, no defaults
# ===========================================================================
...
fg_loaded = False
if use_fangraphs:
    with st.spinner("Scraping FanGraphs..."):
        try:
            fg_df = get_fangraphs_pitch_models()
            if not fg_df.empty:
                pitcher_stats = merge_fangraphs_into_pitchers(pitcher_stats, fg_df)
                fg_loaded = True
        except Exception:
            pass
    if not fg_loaded:
        st.info("FanGraphs scrape failed or returned no data. Continuing without Stuff+/Location+/Pitching+.")
...
src1.metric("Games", len(slate))
src2.metric("Hitters", f"{len(hitter_stats)}")
src3.metric("Pitchers", f"{len(pitcher_stats)}")
src4.metric("Vegas", "✓" if not vegas_df.empty else "—")
src5.metric("FanGraphs", "✓" if fg_loaded else "—")
...
with st.expander("📖 Dashboard Legend", expanded=False):
    ...
    with leg1:
        ...
        st.markdown(
            ...
            "- **kHR:** Lineup HR vs Pitcher\n"
            "- **Proj K:** K projection\n"
            "- **HR Game%:** Probability ≥1 HR today\n"
            "- **Stuff+/Loc+/Pit+:** FanGraphs models (100=avg)"
        )
    ...
    with leg3:
        ...
        st.markdown(
            ...
            "- **Stuff+:** Pitch nastiness (FanGraphs)\n"
            "- **Pitching+:** Combined Stuff+Location"
        )
...
# ===========================================================================
# BUILD PER-GAME CONTEXT - using only real data
# ===========================================================================
game_context_map = {}
        ...
        for _, hrow in matchup_df.iterrows():
            try:
                ...
                p_pa = hr_prob_per_pa(
                    ...
                    ttop_mult=ttop,
                    defense_factor=1.0,
                    min_pa=min_pa_filter,
                )

                if p_pa is None:
                    # Insufficient sample - no projection
                    hr_pa.append(None)
                    hr_game.append(None)
                    verdicts.append("⚪")
                    continue
                ...
                # Verdict from real matchup score only
...
if not pitcher_slate.empty:
    # Add FanGraphs columns if available
    if fg_loaded and "player_name" in pitcher_slate.columns:
        # Already merged at load time - just confirm columns exist
        pass

    # Alert based on real test_score only
    ...
    col_config = {
        ...
        "stuff_plus": st.column_config.NumberColumn("Stuff+", format="%.0f",
            help="FanGraphs pitch quality (100=avg). Real data only - blank if not scraped."),
        "location_plus": st.column_config.NumberColumn("Loc+", format="%.0f",
            help="FanGraphs location quality"),
        "pitching_plus": st.column_config.NumberColumn("Pit+", format="%.0f",
            help="FanGraphs combined Stuff+Location"),
...
st.caption("Real data only. Empty cells = insufficient or unavailable data.")


def render_matchup(df):
    ...
    # Apply hide_no_data filter
    if hide_no_data and "pa" in df.columns:
        df = df[df["pa"].notna() & (df["pa"] > 0)]
        if df.empty:
            st.caption("All players filtered out (no real PA data)")
            return

    # Alert from real test_score
    ...
    col_config = {
        ...
        "hr_game_pct": st.column_config.NumberColumn("HR Game%", format="%.1f%%",
            help="Real probability ≥1 HR. Blank if insufficient sample (PA below threshold)."),
        ...
        "pa": st.column_config.NumberColumn("PA", format="%d",
            help="Plate appearances (real count)"),
        ...
        "sprint_speed": st.column_config.NumberColumn("Sprint", format="%.1f",
            help="Statcast sprint speed ft/sec. League avg ~27."),
        "k_pct": st.column_config.NumberColumn("K%", format="%.1f%%"),
...
for _, game in slate.iterrows():
    ...
    with st.container(border=True):
        ...
        env1.metric(
            ...
            delta=f"Proj K: {away_k_mean:.1f}" if away_k_mean is not None else None,
        )
        env2.metric(
            f"Home ({game['home_team_abbr']})",
            game.get("home_pitcher") or "TBD",
            delta=f"Proj K: {home_k_mean:.1f}" if home_k_mean is not None else None,
        )
        ...
        # Top HR contender - only from real projections
        all_hitters = []
        if ctx.get("away_matchup") is not None and not ctx["away_matchup"].empty:
            all_hitters.append(ctx["away_matchup"].copy())
        if ctx.get("home_matchup") is not None and not ctx["home_matchup"].empty:
            all_hitters.append(ctx["home_matchup"].copy())
        ...
        with tabs[0]:
            render_matchup(ctx.get("away_matchup"))
        with tabs[1]:
            render_matchup(ctx.get("home_matchup"))
        with tabs[2]:
            ...
            for col, side, label in [
                ...
                with col:
                    ...
                    if not pp or pp.get("mean") is None:
                        st.write(f"**{label}** — no real K data available")
                        continue
                    st.markdown(
