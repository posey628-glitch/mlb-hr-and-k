...
APP_VERSION = "2026.06.10-v43.84-pattern-discovery-fetches-live"
...
st.caption(
    f"📦 v43.84 · {_wx_status_emoji} Weather: {_wx_status_label} · "
...
for _, game in slate.iterrows():
    ...
    # version below — duplicated here so matchup tables can display them
    # without needing combined_all to be built first.
    def _add_robbed_hr_cols(m_df):
        try:
            ...
            pass
        return m_df
    away_matchup = _add_robbed_hr_cols(away_matchup)
    home_matchup = _add_robbed_hr_cols(home_matchup)
    ...
    def _apply_pitch_match(matchup_df, opp_p_row):
        ...
        # -2 or less = "shut down" — high-usage pitches the hitter struggles vs
        # Display only.
        def _exposure_flag(v):
            try:
                vi = int(v) if v is not None and not pd.isna(v) else 0
                if vi >= 2:
                    return f"🎯 feast +{vi}"
                if vi <= -2:
                    return f"🛡️ shut down {vi}"
                if vi >= 1:
                    return f"+{vi} edge"
                if vi <= -1:
                    return f"{vi} edge"
                ...
                return ""
        matchup_df["exposure_flag"] = matchup_df["pitch_exposure_edge"].apply(_exposure_flag)
...
if combined_picks is not None and not combined_picks.empty:
    ...
    try:
        ...
        # v43.67 (researcher framework): coverage check for the new columns
        # the Must-Have / Nuclear evaluators read. If avg_dist or barrel_count
        # don't flow from Savant, the Nuclear filter goes mostly dark and
    ...
    if all(c in qualified.columns for c in ["recent_hr", "barrel_pct", "xwoba", "iso"]):
        ...
        if not due_pool.empty:
            ...
            due_cols = [c for c in [
                ...
                # v42r/43+: Robbed HR diagnostics — physics-grounded due signal
                "xhr_neutral", "hr_luck_gap", "hr_conv_ratio",
                ...
                column_config={
                    ...
                    "xhr_neutral": st.column_config.NumberColumn(
                        ...
                        help="Park-neutral expected HRs from barrel quality "
                             "(barrel% × 0.385 × PA). Compare to L15 HR — bigger "
                             "gap = more 'robbed' / due for regression."),
                    "hr_luck_gap": st.column_config.NumberColumn(
                        "Luck Gap", format="%.1f", width="small",
                        help="xHR − actual HRs. Positive = unlucky (contact quality "
                             "says more HRs than they've gotten). Higher = more due."),
                    "hr_conv_ratio": st.column_config.NumberColumn(
                        "Conv", format="%.2f", width="small",
                        help="Actual HRs / xHR. <0.7 = unlucky-quality-contact "
                             "(due for regression). >1.5 = park-propped (fade on road)."),
                },
            )
...
# ============================================================================
# 🆚 HEAD-TO-HEAD COMPARISON (v42k — relocated + column fixes)
# ============================================================================
# Pick 2-4 hitters, see their key stats side-by-side with the leader in each
# category marked 🏆. Display-only — does not affect scoring.
#
# v42k bugfixes (reviewer audit caught all of these):
#   1. RELOCATED: was at line ~6840 BEFORE combined_all was built. Now placed
#      after combined_all + all enrichment passes (pick_score merge, IL
#      detection, slate-wide hr_form, env_boost, etc.) so the data is fully
#      populated when H2H tries to read it.
#   2. "hr_grade" → "grade" (real column name in combined_all)
#   3. "convergence_score" REMOVED (real column is convergence_count, only
#      set on q/picks frame, never merged into combined_all)
#   4. env_boost now set on the all_hitters path so it's present on
#      combined_all (was only set on all_hitters_for_picks → combined_picks)
...
# data card without scrolling through games. Useful for prop bet research
